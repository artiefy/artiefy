...
import { FiAlertCircle, FiCheck, FiDownload, FiMove, FiTrash2, FiUpload, FiX } from 'react-icons/fi';
...
interface Props { onFinished?: (res: unknown) => void }
...
const BulkUploadUsersV2: React.FC<Props> = ({ onFinished }) => {
    ...
    const dbFields = useMemo(() => [
        ...
        { value: 'birthDate', label: 'F. Nacimiento (YYYY-MM-DD)', required: false },

        // académicos / programa
        { value: 'nivelEducacion', label: 'Nivel Educación', required: false },
        { value: 'programa', label: 'Programa', required: false },
        { value: 'fechaInicio', label: 'F. Inicio (YYYY-MM-DD)', required: false },
        ...
        { value: 'inscripcionValor', label: 'Valor inscripción', required: false },
        { value: 'paymentMethod', label: 'Método de pago', required: false },
        { value: 'cuota1Fecha', label: 'Cuota1 fecha (YYYY-MM-DD)', required: false },
        { value: 'cuota1Metodo', label: 'Cuota1 método', required: false },
        { value: 'cuota1Valor', label: 'Cuota1 valor', required: false },
        { value: 'inscripcionOrigen', label: 'Origen inscripción', required: false },
        { value: 'purchaseDate', label: 'Fecha compra (YYYY-MM-DD)', required: false },

        // acudiente / identificación
        { value: 'identificacionTipo', label: 'Tipo identificación', required: false },
        { value: 'identificacionNumero', label: 'Número identificación', required: false },
        { value: 'tieneAcudiente', label: 'Tiene acudiente', required: false },
        { value: 'acudienteNombre', label: 'Acudiente nombre', required: false },
        { value: 'acudienteContacto', label: 'Acudiente contacto', required: false },
        { value: 'acudienteEmail', label: 'Acudiente email', required: false },
    ], []);
    const schemaKeys = useMemo(() => new Set(dbFields.map((d) => d.value)), [dbFields]);
...
- Omitidos por compatibilidad: ${resumen?.omitidosPorCompatibilidad ?? 0}`;
    ...
    const allRequiredMapped = useMemo(
        () => ['firstName', 'lastName', 'email'].every((x) => mappings.map((m) => m.dbField).includes(x)),
        [mappings],
    );
    ...
    const editCell = (rowIdx: number, excelColumn: string, value: string) => {
        setRows((prev) => prev.map((r, i) => (i === rowIdx ? { ...r, [excelColumn]: value } : r)));
    };
                ...
                <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 p-2 sm:p-4">
                    <div className="w-full max-w-[95vw] overflow-hidden rounded-xl bg-white text-gray-800 shadow-2xl">
                        ...
                        <div className="flex items-center justify-between bg-gray-900 px-4 py-3 text-white">
                            <div>
                                <h2 className="text-sm font-semibold sm:text-base">Carga Masiva de Usuarios (v2)</h2>
                                <p className="text-[10px] opacity-80">Tabla editable y opción de quitar candidatos</p>
                            </div>
                            <button
                                onClick={() => {
                                    setOpen(false);
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="relative rounded-lg border-2 border-dashed border-gray-300 p-8 text-center hover:border-gray-400">
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                if (f) {
                                                    try {
                                                        ...
                                                        notify(error?.message || 'No se pudo leer el archivo');
                                                    }
                                                }
                                            }}
                                        />
                                        ...
                                        <p className="text-xs font-medium">
                                            {file ? file.name : 'Arrastra o haz clic para seleccionar .xlsx'}
                                        </p>
                                        <p className="mt-1 text-[11px] text-gray-500">Detectamos encabezados automáticamente</p>
                                    </div>

                                    {/* Plantilla */}
                                    <div className="rounded-md bg-gray-100 p-3">
                                        <div className="flex items-center gap-2 text-gray-800">
                                            <FiAlertCircle className="text-base" /> ¿Plantilla? Puedes descargar un ejemplo.
                                        </div>
                                        <button
                                            onClick={async () => {
                                                const res = await fetch('/api/usersMasive/masiveFiles', { method: 'GET' });
                                                if (!res.ok) return notify('No se pudo descargar la plantilla');
                                ...
                                <div className="space-y-4">
                                            ...
                                            <thead>
                                                <tr className="bg-gray-100">
                                                    {visibleMappings.map((m) => {
                                                        const hit = dbFields.find((d) => d.value === m.dbField);
                                                            ...
                                                            <th
                                                                ...
                                                                onDrop={(e) => {
                                                                    ...
                                                                    if (dragged) {
                                                                        setMappings((prev) =>
                                                                            prev.map((x) =>
                                                                                x.excelColumn === m.excelColumn ? { ...x, dbField: dragged } : x,
                                                                            ),
                                                                        );
                                                                    }
                                                                }}
                                                                className={`min-w-[160px] whitespace-nowrap border p-2 text-left font-semibold ${over ? 'bg-gray-50' : ''
                                                                    }`}
                                                            >
                                                                <div>{m.excelColumn}</div>
                                                                <div
                                                                    className={`mt-2 min-h-[52px] rounded border-2 border-dashed p-2 ${hit ? 'border-emerald-400 bg-emerald-50' : 'border-gray-300 bg-gray-50'
                                                                        }`}
                                                                >
                                                                    {hit ? (
                                                                        <div className="flex items-center justify-between gap-2">
                                                                            <div>
                                                                                ...
                                                                                {hit.required && (
                                                                                    <div className="text-[10px] text-emerald-700">Obligatorio</div>
                                                                                )}
                                                                            </div>
                                                                            <button
                                                                                onClick={() =>
                                                                                    setMappings((prev) =>
                                                                                        prev.map((x) =>
                                                                                            x.excelColumn === m.excelColumn ? { ...x, dbField: '' } : x,
                                                                                        ),
                                                                                    )
                                                                                }
                                                                        ...
                                                                        <div className="text-[10px] italic text-gray-500">Arrastra un campo aquí</div>
                                                                    )}
                                                                </div>
                                                            </th>
                                                        );
                                                    })}
                                                    <th className="min-w-[100px] border p-2 text-left font-semibold">Acciones</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {rows.map((row, idx) => (
                                                    <tr key={idx} className="odd:bg-white even:bg-gray-50">
                                                        {visibleMappings.map((m) => (
                                                            <td key={`${idx}-${m.excelColumn}`} className="min-w-[160px] border p-2">
                                                                <input
                                                                    ...
                                                                    onChange={(e) => editCell(idx, m.excelColumn, e.target.value)}
                                                                />
                                                            </td>
                                                        ))}
                                                ...
                                                {rows.length === 0 && (
                                                    <tr>
                                                        <td className="p-4 text-center text-gray-500" colSpan={visibleMappings.length + 1}>
                                    ...
                                    <div className="rounded-md border p-3">
                                        <div className="mb-2 flex items-center gap-2">
                                            ...
                                            <div className="text-xs font-semibold">Campos disponibles</div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">
                                            {dbFields.map((d) => (
                                                <div
                                                    ...
                                                    className={`cursor-move rounded border p-2 text-center text-[11px] ${d.required ? 'border-red-300' : 'border-gray-300'
                                                        }`}
                                                    title={d.required ? 'Obligatorio' : 'Opcional'}
                                                >
                                                    {d.label}
                                                    {d.required && <div className="text-[10px] text-red-600">* requerido</div>}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            disabled={uploading || !allRequiredMapped || rows.length === 0}
