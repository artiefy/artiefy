...
export default function ModalFormCourse({
  ...
  const handleSubmit = async () => {
    try {
      console.log("📡 Enviando curso al servidor...");
      ...
      const response = await fetch("/api/courses/create", {
        ...
        body: JSON.stringify({
          ...
          requerimientos,
          coverImageKey: "", // 🔹 No enviamos archivo, solo la clave de la imagen vacía
        }),
      });
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
        ...
        <div className="rounded-lg bg-background px-6 text-black shadow-md">
          ...
          <div className="mb-4 w-full rounded border border-primary p-2">
            ...
            Instructor: {user?.firstName && user?.lastName ? `${user.firstName} ${user.lastName}` : user?.emailAddresses?.[0]?.emailAddress ?? "Desconocido"}
