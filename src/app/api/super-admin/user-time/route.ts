...
export async function POST(req: Request) {
  try {
    ...
    if (!userId) {
      console.error("🚨 [POST] Faltan datos o formato inválido:", { userId });
      return NextResponse.json({ error: "Faltan datos o formato inválido" }, { status: 400 });
    }
    ...
    if (existingRecord) {
      ...
      const updatedTime = existingRecord.timeSpent + 60;
      ...
      await db.insert(userTimeTracking).values({
        userId,
        date: today,
        timeSpent: 60, // Guardamos 1 minuto
      }).execute();
