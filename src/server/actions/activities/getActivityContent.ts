...
export async function getActivityContent(lessonId: number): Promise<Activity[]> {
  try {
    ...
    const activitiesWithContent = await Promise.all(
      relatedActivities.map(async (activity) => {
        ...
        return {
          ...
          isCompleted: false, // Esto debería ser determinado basado en el progreso del usuario
          userProgress: 0, // Esto también debería ser calculado basado en el progreso del usuario
        } as Activity
      }),
    )
