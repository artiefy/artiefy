...
export function FiltrosTickets({ filtros, tecnicos, onFiltrosChange, categoriasDisponibles }: FiltrosTicketsProps) {
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="space-y-2">
            <Label>Estado</Label>
            <Select value={filtros.estado} onValueChange={(valor) => onFiltrosChange({ ...filtros, estado: valor })}>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(valor) => onFiltrosChange({ ...filtros, prioridad: valor })}
            >
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(valor) => onFiltrosChange({ ...filtros, tecnicoAsignado: valor })}
            >
              <SelectTrigger>
                ...
                <div className="flex items-center space-x-2" key={categoria}>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        onFiltrosChange({ ...filtros, categorias: [...filtros.categorias, categoria] })
                      } else {
                        onFiltrosChange({
                          ...filtros,
                          categorias: filtros.categorias.filter((cat) => cat !== categoria),
                        })
                      }
                    }}
                  />
                  <Label htmlFor={`filtro-categoria-${categoria}`}>{categoria}</Label>
                </div>
              ))}
            </div>
          </div>
