...
// Endpoint to handle the complete flow
app.post('/api/complete-subscription', async (req, res) => {
  try {
    ...
    const customerInfo = { ...req.body.customer_info, token_card: token.id };
    ...
    const subscriptionInfo = {
      ...req.body.subscription_info,
      id_plan: req.body.subscription_info.id_plan,
    ...
    // Step 4: Charge subscription
    const chargeInfo = {
      ...req.body.subscription_info,
      id_plan: req.body.subscription_info.id_plan,
      customer: customer.data.customerId,
      token_card: token.id,
      ip: req.ip, // Assuming the client's IP is available in req.ip
    };
    const charge = await epayco.subscriptions.charge(chargeInfo);
    console.log('Payment ID:', charge.id);

    res.json({ token, customer, subscription, charge });
  } catch (err) {
    console.error('Error en el flujo completo de suscripción:', err);
    res.status(500).json({ error: err.message });
  }
});
