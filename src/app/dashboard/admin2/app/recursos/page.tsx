...
export default function Recursos() {
  ...
  const handleAddRecurso = useCallback(
    (nuevoRecurso: Omit<Recurso, 'id' | 'descargas'>) => {
      setRecursos((prev) => [
        ...prev,
        { ...nuevoRecurso, id: prev.length + 1, descargas: 0 },
      ]);
    },
    []
  );
  ...
  const handleEditRecurso = useCallback((recursoEditado: Recurso) => {
    setRecursos((prev) =>
      prev.map((recurso) =>
        recurso.id === recursoEditado.id ? recursoEditado : recurso
      )
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <DashboardMetrics
          metrics={[
            ...
            {
              title: 'Cursos con Recursos',
              value: new Set(recursos.map((r) => r.curso)).size.toString(),
              icon: BookOpen,
              href: '/cursos',
            },
            {
              ...
              value: recursos
                .reduce((acc, r) => acc + r.descargas, 0)
                .toString(),
              icon: Download,
              href: '/analisis',
            },
            ...
            <GenericForm
              fields={[
                { name: 'nombre', label: 'Nombre del Recurso', type: 'text' },
                {
                  ...
                  type: 'select',
                  options: ['documento', 'video', 'enlace', 'youtube', 'drive'],
                },
      ...
      <GenericTable
        ...
        actions={(row) => {
          const recurso = row as unknown as Recurso;
          return (
            <>
              <Button
                variant="outline"
                className="mr-2"
                onClick={() => handleViewRecurso(recurso)}
              >
                Ver
              </Button>
              <Dialog
