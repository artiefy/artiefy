...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (materias.length === 0) {
            return NextResponse.json({ error: 'No se encontraron materias para este curso' }, { status: 404 });
        }
        ...
        return NextResponse.json({ error: 'Error al obtener las materias' }, { status: 500 });
    }
}
