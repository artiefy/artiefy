...
import { FaCalendar, FaChevronDown, FaChevronUp, FaClock, FaHome, FaStar, FaUserGraduate } from 'react-icons/fa';
            ...
            <div className="flex flex-wrap gap-6 p-6">
              <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="flex items-center">
                      {Array.from({ length: 5 }).map((_, index) => (
                        <FaStar key={index} className={`size-5 ${index < Math.floor(course.rating ?? 0) ? 'text-yellow-400' : 'text-gray-300'}`} />
                      ))}
                ...
                <div className="prose mb-8 max-w-none">
                  <p className="leading-relaxed text-gray-700">{course.description ?? 'No hay descripción disponible.'}</p>
                </div>
                ...
                <div className="mt-8 p-4">
                  <h2 className="mb-6 text-2xl font-bold text-background">Contenido del curso</h2>
                  <div className="space-y-4">
                    {course.lessons.map((lesson) => (
                      <div key={lesson.id} className="overflow-hidden rounded-lg border bg-gray-50 transition-colors hover:bg-gray-100">
                        <button className="flex w-full items-center justify-between px-6 py-4" onClick={() => toggleLesson(lesson.id)}>
                          <div className="flex items-center">
                            <span className="font-medium text-background">{lesson.title}</span>
                            <span className="ml-4 text-sm text-gray-500">{lesson.duration} mins</span>
                          </div>
                          {expandedLesson === lesson.id ? <FaChevronUp className="text-gray-400" /> : <FaChevronDown className="text-gray-400" />}
                        </button>
                ...
                <div className="flex h-[400px] flex-col">
                  <div className="border-b px-4 py-3 dark:border-zinc-700">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="rounded-full bg-green-500 px-2 py-1 text-xs text-white">Online</div>
                    </div>
                  </div>
                  <div className="flex flex-1 flex-col space-y-2 overflow-y-auto p-3" id="chatDisplay">
                    <div className="max-w-xs self-end rounded-lg bg-secondary px-3 py-1.5 text-sm text-white">Hello! How can I assist you today?</div>
                    <div className="max-w-xs self-start rounded-lg bg-zinc-500 px-3 py-1.5 text-sm text-white">Hello! I need a Chatbot!</div>
                  </div>
