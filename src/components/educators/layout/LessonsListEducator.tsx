...
import StudentsModal from '../../super-admin/modals/studentModal';
...
const LessonsListEducator: React.FC<LessonsListProps> = ({
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    // Persistir cambios: PUT /api/educadores/lessons/[id] con { orderIndex }
    try {
      await Promise.all(
        withNewOrder.map((l) =>
          fetch(`/api/educadores/lessons/${l.id}`, {
            method: 'PUT',
  ...
  const handleDragEndVisible = async (result: DropResult) => {
    ...
    try {
      ...
      const response = await fetch('/api/educadores/lessons/reorder', {
        method: 'POST',
        headers: {
