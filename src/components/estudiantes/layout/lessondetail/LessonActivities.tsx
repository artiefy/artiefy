...
const LessonActivities = ({
	...
	return (
		<div className="w-72 bg-background p-4 shadow-lg">
			...
			{activity?.id ? ( // Changed from activity? to activity?.id to check for valid activity
				<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
					...
					{!hasQuestions ? (
						<div className="mt-4 text-center text-sm text-red-500">
							No se pudieron cargar las preguntas de la actividad.
							<br />
							Key: activity:{activity.id}:questions
						</div>
					) : null}
				...
				<div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
					...
					<p className="mt-2 text-center text-sm text-gray-500">
						La actividad existe pero aún no tiene contenido.
					</p>
				</div>
			)}
			{activity && (
