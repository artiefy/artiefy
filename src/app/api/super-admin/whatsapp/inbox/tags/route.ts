...
export async function GET(req: Request) {
    ...
    if (!waid) return NextResponse.json({ error: 'waid requerido' }, { status: 400 });
...
export async function POST(req: Request) {
    ...
    if (!waid || !tagId) return NextResponse.json({ error: 'waid y tagId requeridos' }, { status: 400 });
...
export async function DELETE(req: Request) {
    const body = (await req.json()) as { waid?: string | number; tagId?: string | number };
    ...
    if (!waidStr || Number.isNaN(tagNum)) {
        return NextResponse.json({ error: 'waid y tagId requeridos' }, { status: 400 });
    }
