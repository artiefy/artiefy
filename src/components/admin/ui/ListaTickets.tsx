...
export const ListaTickets = ({ tickets, onSeleccionarTicket }: ListaTicketsProps) => {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Avatar className="w-6 h-6">
                  ...
                  <AvatarFallback>{ticket.asignadoA ? ticket.asignadoA.charAt(0).toUpperCase() : "NA"}</AvatarFallback>
                </Avatar>
                <span className="text-sm">{ticket.asignadoA ? `Asignado a: ${ticket.asignadoA}` : "Sin asignar"}</span>
              </div>
              {ticket.tiempoEstimado && (
                <span className="text-sm text-muted-foreground">Estimado: {ticket.tiempoEstimado}h</span>
              )}
            </div>
            <div className="flex flex-wrap gap-1 mt-2">
              {ticket.categorias.map((categoria) => (
