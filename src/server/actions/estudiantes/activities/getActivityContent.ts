	...
	try {
		...
		const activitiesWithContent = await Promise.all(
			relatedActivities.map(async (activity) => {
				...
				if (!activityContent) {
					console.log(
						`No se encontró contenido para la actividad ${activity.id}`
					);
					return null;
				}
				...
				if (typeof activityContent === 'string') {
					try {
						...
						console.error(
							`Error al parsear el contenido de la actividad ${activity.id}:`,
							error
						);
						return null;
					}
					...
					console.error(
						`Contenido inesperado para la actividad ${activity.id}:`,
						activityContent
					);
					return null;
				}
		...
		if (validActivities.length === 0) {
			console.log(
				`No se encontraron actividades con contenido válido para la lección ${lessonId}`
			);
		} else {
			console.log(
				`Se encontraron ${validActivities.length} actividades con contenido para la lección ${lessonId}`
			);
		}
