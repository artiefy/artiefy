...
export default function Tutores() {
  ...
  const handleEditTutor = (tutorEditado: Omit<Tutor, 'id' | 'estudiantes' | 'calificacion'>) => {
    setTutores(tutores.map(tutor => 
      tutor.id === selectedTutor?.id 
        ? { ...tutor, ...tutorEditado, estudiantes: tutor.estudiantes, calificacion: tutor.calificacion }
        : tutor
    ))
  ...
  return (
    <div className="space-y-6 p-8 bg-background">
      <h2 className="text-3xl font-bold tracking-tight text-foreground">Gestión de Tutores</h2>
        ...
        <CardContent className="flex justify-between items-center">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button><Plus className="mr-2 h-4 w-4" /> Agregar Tutor</Button>
            </DialogTrigger>
            ...
            <TableBody>
              {filteredTutores.map((tutor) => (
                <TableRow key={tutor.id}>
                  ...
                  <TableCell>
                    <Button variant="outline" size="sm" className="mr-2" onClick={() => setSelectedTutor(tutor)}>Ver Detalles</Button>
                    <Dialog>
                      <DialogTrigger asChild>
