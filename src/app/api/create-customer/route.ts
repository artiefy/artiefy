...
export async function POST(request: Request) {
  try {
    ...
    const { docType, docNumber, name, lastName, email, cellPhone, phone, address, city, cardTokenId } = await request.json();
    ...
    if (!data.data || !data.data.customerId) {
      console.error('customerId no recibido:', data);
      throw new Error('customerId no recibido');
    }
