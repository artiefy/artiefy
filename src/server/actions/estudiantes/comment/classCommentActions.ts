...
export async function addClassComment(
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      return { success: false, message: "Error desconocido al agregar comentario" }
    }
  }
}

export async function getCommentsByLessonId(lessonId: number): Promise<{ comments: ClassComment[] }> {
  try {
...
export async function editClassComment(
  ...
  try {
    ...
    if (error instanceof Error) {
      return { success: false, message: `Error al editar comentario: ${error.message}` }
    } else {
      return { success: false, message: "Error desconocido al editar comentario" }
    }
  }
}

export async function deleteClassComment(commentId: string): Promise<{ success: boolean; message: string }> {
  try {
    const user = await currentUser()
    ...
    if (error instanceof Error) {
      return { success: false, message: `Error al eliminar comentario: ${error.message}` }
    } else {
      return { success: false, message: "Error desconocido al eliminar comentario" }
    }
  }
}

export async function likeClassComment(commentId: string): Promise<{ success: boolean; message: string }> {
  try {
    const user = await currentUser()
    ...
    if (error instanceof Error) {
      return { success: false, message: `Error al modificar me gusta: ${error.message}` }
    } else {
      return { success: false, message: "Error desconocido al modificar me gusta" }
    }
  }
}
