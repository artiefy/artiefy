...
export default function Page() {
	...
	const handleCreateOrUpdateCourse = async (
		...
		if (id) {
			await updateCourse(Number(id), {
				...
				nivelid: Number(nivelid), // changed from
				rating,
				instructor,
			} as CourseData);
		} else {
	...
	// Manejo de la  del curso en el modal si no es null
	// const  = (: number) => {
	//     setEditingCourse((prev: CourseData | null) =>
	//         prev ? { ...prev, : Number() } : prev
	//     );
	// };
			...
			<div className="p-6">
				...
				{isModalOpen && (
					<ModalFormCourse
						...
						nivelid={editingCourse?.nivelid ?? 0} // changed from 
						setNivelid={(
							nivelid: number // changed from 
						) =>
							setEditingCourse(
								(prev) => (prev ? { ...prev, nivelid } : null) // changed from 
							)
						}
