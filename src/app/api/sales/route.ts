...
export async function POST(req: NextRequest) {
	...
	try {
		...
		if (runStatus === 'failed') {
			return NextResponse.json({ error: 'Error en la ejecución del asistente' }, { status: 500 });
		}
		...
		const finalMessage = messagesData.data[0]?.content[0]?.text?.value ?? 'No hubo respuesta.';
