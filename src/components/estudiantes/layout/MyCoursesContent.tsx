...
/**
 * Enrolled courses + programs tabbed grids.
 * Shared between the "Mi Aprendizaje" page and the profile "Cursos" tab so the
 * card markup lives in a single place.
 */
export default function MyCoursesContent({
  courses,
  programs,
}: MyCoursesContentProps) {
  return (
    <Tabs.Root defaultValue="cursos">
      <Tabs.List
        className="
      ...
      >
        <Tabs.Trigger
          value="cursos"
          className="
            inline-flex items-center justify-center gap-2 rounded-[13px] px-4
            py-1.5 text-sm font-medium whitespace-nowrap
            ring-offset-background transition-all
            hover:bg-[#22C4D3] hover:text-black
            focus-visible:bg-[#22C4D3] focus-visible:text-black
