...
import { getAllMaterias, getMateriaById, createMateria, updateMateria, deleteMateria } from '~/models/super-adminModels/materiaModels';
...
export async function GET(req: NextRequest) {
  ...
  if (id) {
    try {
      ...
      return NextResponse.json({ error: 'Failed to fetch materia', details: errorMessage }, { status: 500 });
    }
  } else {
    try {
      ...
      const errorMessage = (error as Error).message;
      return NextResponse.json({ error: 'Failed to fetch materias', details: errorMessage }, { status: 500 });
    }
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create materia', details: errorMessage }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const errorMessage = (error as Error).message;
    return NextResponse.json({ error: 'Failed to delete materia', details: errorMessage }, { status: 500 });
  }
}