...
export async function POST() {
  try {
    ...
    const manualVideoUrl = 'https://s3.us-east-2.amazonaws.com/artiefy-upload/uploads/video-corporativo1080-1747179011696-ab77ff51-074d-4f6c-abdf-1be20e8a79fc-1747786015742-3f62f9ed-eaeb-4442-b444-3bd66610bdc4.mp4'; // 👈 Tu video URL

    // 🔹 MODO MANUAL: solo si lessonId !== 0 y URL no está vacío
    if (manualLessonId !== 0 && manualVideoUrl.trim()) {
      const redisKey = `transcription:lesson:${manualLessonId}`;
      const alreadyExists = await redis.get(redisKey);

      if (alreadyExists) {
        console.log(`[TRANSCRIPCIÓN] 🟡 Ya existe transcripción para lección ${manualLessonId}`);
        return NextResponse.json({
      ...
      try {
        ...
        if (!check.ok) {
          console.error(`[TRANSCRIPCIÓN] ❌ Video no accesible. Status: ${check.status}`);
          return NextResponse.json(
        ...
        console.error(`[TRANSCRIPCIÓN] ❌ Error al verificar el video manual:`, err);
        return NextResponse.json({ error: 'Error al verificar el video' }, { status: 400 });
      }
      ...
      try {
        ...
        if (!Array.isArray(response.data)) {
          console.error(`[TRANSCRIPCIÓN] ❌ Formato inválido para lección ${manualLessonId}:`, response.data);
          return NextResponse.json({ error: 'Formato de transcripción no válido' }, { status: 422 });
        }

        await redis.set(redisKey, response.data);
        console.log(`[TRANSCRIPCIÓN] ✅ Guardada transcripción para lección ${manualLessonId}`);
        ...
        return NextResponse.json({ error: 'Error procesando la transcripción manual' }, { status: 500 });
      }
    }
    ...
    const lessonsToProcess = allLessons.filter((lesson) => {
      return (
        ...
        !alreadyProcessedIds.has(lesson.id.toString())
      );
    });

    if (lessonsToProcess.length === 0) {
      console.log('[TRANSCRIPCIÓN] ✅ No hay lecciones nuevas por transcribir.');
      return NextResponse.json({
    ...
    for (const lesson of lessonsToProcess) {
      ...
      try {
        ...
        if (!check.ok) {
          console.error(`[TRANSCRIPCIÓN] ❌ Video no accesible para lección ${lessonId}. Status:`, check.status);
          continue;
        }
      } catch (error) {
      ...
      try {
        ...
        if (!Array.isArray(response.data)) {
          console.error(`[TRANSCRIPCIÓN] ❌ Formato inválido para lección ${lessonId}:`, response.data);
          continue;
        }
