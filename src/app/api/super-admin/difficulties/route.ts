...
export async function POST(req: Request) {
    try {
        ...
        const result = await db.insert(nivel).values({ name, description }).returning();
        return NextResponse.json(result);
    } catch (error) {
...
export async function PUT(req: Request) {
    try {
        ...
        const result = await db.update(nivel).set({ name, description }).where(eq(nivel.id, id)).returning();
        return NextResponse.json(result);
    } catch (error) {
