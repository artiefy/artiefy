...
import type {
  ...
  Course,
  LessonWithProgress,
  UserActivitiesProgress, // Importamos UserActivitiesProgress
} from "~/types"
...
export default function LessonDetails({
    ...
    try {
      ...
      if (!activity) {
        ...
        if (result.success && "nextLessonId" in result) {
          setLessonsState((prevLessons) =>
            prevLessons.map((l) => (l.id === result.nextLessonId ? { ...l, isLocked: false } : l)),
          )
          toast({
  ...
  const handleProgressUpdate = async (videoProgress: number) => {
    ...
    if (roundedProgress > progress && roundedProgress < 100) {
      ...
      try {
        ...
        setLessonsState((prevLessons) =>
          prevLessons.map((l) => (l.id === lesson.id ? { ...l, porcentajecompletado: roundedProgress } : l)),
        )
      } catch (error) {
        console.error("Error al actualizar el progreso de la lección:", error)
      }
  ...
  const handleActivityCompletion = async () => {
    ...
    try {
      ...
      setLessonsState((prevLessons) => prevLessons.map((l) => (l.id === lesson.id ? { ...l, isCompleted: true } : l)))
