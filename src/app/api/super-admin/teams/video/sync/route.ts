  ...
  for (const [meetingId, recs] of recordingsByMeeting.entries()) {
    try {
      ...
      const sortedRecs = recs
        ...
        .sort(
          (a, b) =>
            parseCreatedMs(a.createdDateTime) -
            parseCreatedMs(b.createdDateTime)
        );
      ...
      if (
        primaryKey &&
        !synced.some(
          (s) => s.meetingId === meetingId && s.videoKey === primaryKey
        )
      ) {
        synced.push({
          meetingId,
      ...
      if (
        secondaryKey &&
        !synced.some(
          (s) => s.meetingId === meetingId && s.videoKey === secondaryKey
        )
      ) {
        synced.push({
          meetingId,
          ...
          createdAt:
            sortedRecs[1]?.createdDateTime ?? sortedRecs[0]?.createdDateTime,
          isSecondary: true,
          source: 'graph',
        });
      }
      ...
      for (const recording of sortedRecs) {
        ...
        const updatePayload = !primaryKey
          ? { video_key: videoKey }
