...
export default function CourseDetails({
      ...
      if (userId) {
        ...
        if (isUserEnrolled && !loading) {
          try {
            ...
            toast.error('No se pudo cargar el progreso de las lecciones');
          }
        }
      }
    };
  ...
  const handleEnroll = async () => {
    if (!isSignedIn) {
      toast.error('Debes iniciar sesión para inscribirte en este curso.');
    ...
    if (!isActive) {
      toast.error('Necesitas una suscripción activa para acceder a este curso.');
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success('Te has inscrito correctamente al curso.');
      } else {
