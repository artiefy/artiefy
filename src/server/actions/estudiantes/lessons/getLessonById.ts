...
import { lessons, userLessonsProgress, userActivitiesProgress, enrollments } from '~/server/db/schema';

import type { Lesson, Activity, Course } from '~/types';
...
export async function getLessonById(
    ...
    try {
        ...
        const transformedLessons: Lesson[] = lesson.course.lessons.map(l => ({
            ...l,
            porcentajecompletado: lessonsProgress.find(p => p.lessonId === l.id)?.progress ?? 0,
            userProgress: lessonsProgress.find(p => p.lessonId === l.id)?.progress ?? 0,
            isCompleted: lessonsProgress.find(p => p.lessonId === l.id)?.isCompleted ?? false,
            isLocked: lessonsProgress.find(p => p.lessonId === l.id)?.isLocked ?? true,
        ...
        const transformedLesson: Lesson = {
            ...
            resourceNames: lesson.resourceNames ? lesson.resourceNames.split(',') : [],
