...
const ModalFormMateria: React.FC<ModalFormMateriaProps> = ({
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newMateria: Materia = (await response.json()) as Materia;
            onCreate(newMateria);
            onClose();
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`/api/super-admin/materias?id=${editingMateria?.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(materiaData),
            });
            ...
            const updatedMateria: Materia = (await response.json()) as Materia;
            onUpdate(updatedMateria);
            onClose();
            toast.success('Materia actualizada exitosamente');
        } catch (error) {
            toast.error('Error', {
                description: 'Error al actualizar la materia: ' + (error as Error).message,
            });
            console.error('Error:', error);
        }
    };
