...
const ModalFormCourse: React.FC<CourseFormProps> = ({
  ...
  console.log('🎨 Educators disponibles:', educators.length);
  ...
  useEffect(() => {
    if (editingCourseId && instructors && instructors.length > 0) {
      setInstructors(instructors);
      // Actualizar el estado local cuando cambien los instructores
      setModifiedFields((prev) => new Set(prev).add('instructors'));
    }
  }, [editingCourseId, instructors, educators]);
  ...
  // ✅ Efecto para cargar el courseTypeId cuando se abre el modal para editar
  useEffect(() => {
    if (editingCourseId && isOpen) {
      console.log(
        '📋 [SYNC COURSE_TYPE] Sincronizando para editingCourseId:',
        editingCourseId,
        'courseTypeId prop:',
        {
          value: courseTypeId,
          isArray: Array.isArray(courseTypeId),
          length: Array.isArray(courseTypeId) ? courseTypeId.length : 'N/A',
        }
      );

      // Asegurarse de que courseTypeId es un array
      if (Array.isArray(courseTypeId)) {
        // Filtrar valores válidos
        const validTypeIds = courseTypeId.filter(
          (id) => id !== null && id !== undefined && id !== 0
        );

        console.log('✅ [SYNC COURSE_TYPE] setCourseTypeId a:', {
          validTypeIds,
          count: validTypeIds.length,
        });
        setCourseTypeId(validTypeIds);
      } else if (typeof courseTypeId === 'number' && courseTypeId !== 0) {
        console.log('✅ [SYNC COURSE_TYPE] Convirtiendo número a array:', [
          courseTypeId,
        ]);
        setCourseTypeId([courseTypeId]);
      } else {
        console.log(
          '⚠️ [SYNC COURSE_TYPE] courseTypeId vacío, seteando array vacío'
        );
        setCourseTypeId([]);
      }
    }
                ...
                <div className="space-y-3 rounded-lg border border-primary p-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="mt-2 w-full rounded border bg-background p-2 text-sm text-white"
                    />

                    {showParametroList && (
                      <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded border border-gray-600 bg-background shadow-lg">
                        {existingParametros
                          .filter((p) =>
                            p.name
                              .toLowerCase()
