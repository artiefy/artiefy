				...
				<div className="space-y-6">
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
							{safeMaterias.map((materia, index) => {
								const curso = materia.curso;
								return (
									<div
										key={`${materia.id}-${curso.id}-${index}`}
										className="relative h-48 w-full md:h-64"
									>
										<Image
											src={
												curso.coverImageKey && curso.coverImageKey !== 'NULL'
													? `${process.env.NEXT_PUBLIC_AWS_S3_URL}/${curso.coverImageKey}`
													: 'https://placehold.co/600x400/01142B/3AF4EF?text=Artiefy&font=MONTSERRAT'
											}
											alt={curso.title}
											fill
										...
										<div className="absolute inset-x-0 bottom-0 bg-black/50 p-2 text-white">
											<Link href={`/estudiantes/cursos/${curso.id}`}>
