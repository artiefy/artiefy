...
import { courses, lessons, enrollments, categories, modalidades } from '~/server/db/schema';
...
export const getTotalStudents = async (course_id: number): Promise<number> => {
  const result = await db.select({ totalStudents: count() }).from(enrollments).where(eq(enrollments.courseId, course_id));
