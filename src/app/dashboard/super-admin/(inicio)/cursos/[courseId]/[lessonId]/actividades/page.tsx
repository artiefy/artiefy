...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/educadores/actividades/${activityId}`)
      ...
      .then((data: ActivityDetailsAPI) => {
        ...
        // precargar % disponible real desde el servidor (excluyendo actividad actual)
        if (parametroId > 0) {
          fetch('/api/educadores/actividades/actividadesByLesson', {
            ...
            .then((d: { disponible?: number }) =>
              setPorcentajeDisponible(d.disponible ?? 100 - pesoValor)
            )
            .catch(() => setPorcentajeDisponible(100 - pesoValor));
        }
      })
      .catch((err: unknown) => {
  ...
  const handleParametroChange = async (parametroId: number) => {
    ...
    // Fetch de porcentaje disponible (excluyendo actividad actual si estamos editando)
    const response = await fetch(
      '/api/educadores/actividades/actividadesByLesson',
      {
        method: 'POST',
        ...
        body: JSON.stringify({
          parametroId,
          porcentaje: 0,
          ...(activityId ? { actividadId: Number(activityId) } : {}),
        }),
      }
    );
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end">
                <Button
                  onClick={() =>
                    router.push(
                      `/dashboard/super-admin/cursos/${courseIdNumber}/${lessonIdNumber}/actividades/${uploadActivityId}`
                    )
                  }
                  className="
                    ...
                    <>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full rounded-lg border border-slate-200 bg-white p-2 pr-9 text-black outline-none"
                          onChange={(e) =>
                            setFormData({
                              ...formData,
