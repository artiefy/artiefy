...
const ResponsiveSidebar = ({ children }: ResponsiveSidebarProps) => {
    ...
    const { isInactivePopupOpen, handleContinue } = usePageTimeTracker(
        user?.id ?? null
    );
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed top-0 left-0 h-screen w-64 border-r bg-background pt-20 transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} z-30 md:translate-x-0`}
            >
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <li key={item.id}>
                                <Link
                                    href={item.link}
                                    className={`flex items-center space-x-2 rounded-lg p-2 transition-all duration-300 ${isActive ? 'bg-primary text-[#01142B]' : 'text-gray-600 hover:bg-secondary hover:text-white'}`}
                                >
                                    {item.icon}
