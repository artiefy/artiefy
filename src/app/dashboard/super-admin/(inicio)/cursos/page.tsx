...
import { getCourses, createCourse, updateCourse } from '~/server/queries/queries';
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetchCourses().catch((error: Error) => console.error("Error cargando cursos:", error));
  }, []);
  ...
  const handleCreateOrUpdateCourse = async (
    ...
    try {
      ...
      const instructor = user?.fullName ?? user?.emailAddresses[0]?.emailAddress ?? "Desconocido"; // ✅ Asegurar instructor
  
      if (id) {
        await updateCourse(Number(id), { title, description: description ?? '', coverImageKey: coverImageKey ?? '', categoryid: Number(categoryid), modalidadesid: Number(modalidadesid), dificultadid: Number(dificultadid), requerimientos: requerimientos ?? '', instructor } as CourseData);
      } else {
    ...
    <SuperAdminLayout> {/* ✅ Envolver dentro del layout para que el sidebar no desaparezca */}
      <div className="p-6">
        ...
        {courses.length === 0 ? (
          ...
          <CourseListAdmin courses={courses} onEditCourse={setEditingCourse} onDeleteCourse={() => {}} />
        )}
        ...
        {isModalOpen && (
          <ModalFormCourse
            ...
            setRequerimientos={(requerimientos) => setEditingCourse((prev) => prev ? { ...prev, requerimientos } : null)}
            categoryid={editingCourse?.categoryid ?? 1}
            setCategoryid={(categoryid) => setEditingCourse((prev) => prev ? { ...prev, categoryid } : null)}
            modalidadesid={editingCourse?.modalidadesid ?? 1}
            setModalidadesid={(modalidadesid) => setEditingCourse((prev) => prev ? { ...prev, modalidadesid } : null)}
            dificultadid={editingCourse?.dificultadid ?? 1}
            setDificultadid={(dificultadid) => setEditingCourse((prev) => prev ? { ...prev, dificultadid } : null)}
